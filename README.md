...
- **`settings.json`'s `hooks.SessionStart` points at a script this repo doesn't ship — that's expected, not a broken reference.** The script belongs to herdr's own Claude Code integration. Herdr owns and overwrites it, so dotfiles deliberately leaves it out of what it manages here.
