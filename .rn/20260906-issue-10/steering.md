...
- リポジトリの `CLAUDE.md` が、役割 1 文 + 指示 7 行(英語)を持っている。Claude への命令形で
...
- 指示が守れなかったときにそれと分かる程度に具体的で、「適切に」「正しく」のような
  判定できない語で書かれていない
- 本文が 8 行(役割 1 文 + 指示 7 行)で、見出しやタイトルが足されていない。plan gate で合意した
  本文(`claude-md-agreed.md`)から、手順に属する 2 行を落とし、意味を変えない英語の手直しを
  加えたものになっている
