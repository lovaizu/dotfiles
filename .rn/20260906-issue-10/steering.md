...
- リポジトリの `CLAUDE.md` が、plan gate で合意した本文(役割 1 文 + 指示 9 行、英語)を持っている。
  Claude への命令形で人間は the user と書く。本文の中身: the user はドラフトの目的と意図を
  伝え判断する / 目的を言語化して合意 / 最終成果物から逆算
  して進め方を合意 / 小さく作って実物で裏を取る / 判断は結論・根拠・選択肢と推奨 1 つ / 推測で
...
- 持ち運びたい指示の元ネタは、セッション記録 `~/.claude/projects/**/*.jsonl` に残る私自身の
  ...
  5 件を元ネタにする案は却下済み(2026-09-06)。抽出した姿勢を plan gate で 6 回改訂し、最終形は
...
- 指示が守れなかったときにそれと分かる程度に具体的で、「適切に」「正しく」のような
  判定できない語で書かれていない
- 本文が plan gate で合意した 10 行と一致し、見出しやタイトルが足されていない
