...
マージし、**アプリ自身が要素を書き足す配列**は id で1件ずつ突き合わせる: WT の `profiles.list`
(`guid`)・`actions`(`id`)・`schemes`(`name`)、Claude Code の `hooks.<Event>`(`matcher`)と
その要素の中の `hooks` 配列(`command`)。いずれも「WT が WSL ディストロごとにプロファイルを、
UI のキー割り当てごとに action を書き足す」「Claude Code が `/config` で設定したフックを、
dotfiles と同じ matcher の下にも書き足す」ので、配列を葉として置換するとそのマシンにしか無い
要素が消える。それ以外の配列は葉のまま(消えるときは警告を出す)。書き込みは同一ディレクトリの
一時ファイル + `os.replace` で、プロセスが途中で止まっても配置先は旧内容か新内容のどちらかになる。

実装は `setup.sh` の `merge_json` / `merge_toml` が `lib/merge.py` を呼ぶ形(#13 で切り出し。
それまでは setup.sh に埋め込まれていた)。回帰スイートとファズは `tests/`。`jq` も `tomllib` も
...
終末は4系統。setup 全体を止めるのは1番目だけで、残りは終了コード 0 / 3 / 4 / 5 / 6 に割れる:

1. **src が壊れている** — dotfiles 側のバグなので stderr に出して中断する(終了コードは上の5つの
   どれでもない = setup.sh がバグとして扱う)。
2. **マージした / 新規に置いた** — 成功。dst があってマージしたら `0`、dst が無く src をそのまま
   置いたら `5`。
3. **dst の構造が壊れている** — バックアップを取り、dotfiles の内容で復旧し、失ったものと
   バックアップの位置を `WARNING:` で伝えて続行する(`3`)。報告は**書き込みが成功したあと**に
   出す(書けなかったときに「置き換えました」と言わないため)。
4. **マージできない** — dst を一切書かずに警告して続行する。dst があってそのまま残したなら `4`、
   dst が無く新規作成もできなかったなら `6`。「dst に不正な内容を書かない」は dst を触らない
   ことで完全に達成でき、1台の壊れた設定が他の全ファイルの配置を止める理由は無い。

3と4の境界は**「文書そのものが不正」か「読めなかったのは自前パーサの側」か**で引く。終端しない
文字列・閉じない括弧・キーでもヘッダでもない行・キーの二重定義・テーブルの二重宣言・TOML が
どこにも許さない制御文字は、どう読んでも拒まれる形なので結末3。`[[array of tables]]` のように
自前パーサが構造を追えないだけのものは結末4 — パーサの不足を「あなたの設定が壊れている」と
言い換えて破棄しないため。深すぎるネスト(python の再帰上限)も結末4に倒す。

**値の綴りは検査しない。** 裸の語が本当に真偽値・数値・日付かは見ず、値がどこで終わるかだけを見て
バイト列のまま運ぶ。以前は検査しており、その規則の穴(TOML 1.1 の綴り、うるう秒、`\e` エスケープ)が
そのまま「アプリは読めるのに merge が触るのを拒む」= そのマシンが古い dotfiles 値のまま据え置かれる、
という形で表に出た。herdr も Claude Code も自分でパースして不正なら言うので、キー構造を保つ方を
この merge の担当にする。**結果として、元から不正だった dst が `0` で返り不正なまま、ということが
起こりうる。起こらないのは「有効だった dst が無効になって返る」方**。

破れの検出: 書き込む前に必ず自分のパーサで読み直し、キーの重複・テーブルの二重宣言・キーとテーブルの
衝突・開いたままの文字列や括弧・禁止された制御文字があれば書かない(=結末4)。ここでの「書かない」は
マージ**出力**の話で、dst 自体の扱いは上の3と4の境界で決まる。加えてタスク検証で、実物の
`~/.config/herdr/config.toml` のコピーに対する `herdr config check`(`XDG_CONFIG_HOME` で読み先を
差し替え、負のコントロール付き)の往復、実物の WT settings.json × 別マシン想定 dst の往復、
1バイト変異ファズ(「**有効な** dst を無効にして返した」が0件であること)を記録する。

境界: 保証するのは**キー構造が TOML / JSON として合法**であることまでで、値の綴りも、アプリが
その**意味**を受け入れるかも保証しない(未知のキーや型違いの値は src / dst のまま運ばれる)。
`[[array of tables]]` は非対応(管理対象の config に該当なし)。id で突き合わせる5つの配列
以外は葉として丸ごと置き換わるので、dst 固有の要素は消える — 消えるときは警告を出す。
`hooks.<Event>` の要素の中の `hooks` 配列も `command` で突き合わせる。当初は「1つの matcher に
対する定義そのものは dotfiles が持ち主」として葉のままにしていたが、`matcher: "*"` は
SessionStart のごく普通の綴りで、Claude Code 自身の `/config` がまさにその配列に書き込む。
dotfiles だけの持ち物ではないので、他の4つと同じ扱いに揃えた(#7 の設計判断の意図的な差し戻し、
ユーザー承認済み)。JSONC のコメントは JSON マージを通ると失われる(警告を出す)。改行コードも
JSON では保たれない — 文書を組み直して書くので常に LF になる(こちらは警告しない)。

**マージには削除の概念が無い**。反映されるのは「src にあるキー」だけなので、dotfiles からキーを
消しても、既に配られた dst からは消えない — 一度配った値はそのマシンに残り続ける。設定をやめたい
ときは、キーを消すのではなく既定値を明示的に書いて配るか、手で消してもらう必要がある。
これは id で突き合わせる配列の要素にも当てはまり、上の変更でその1段内側まで届くようになった:
dotfiles がフックの `command` の綴りを変えると、新しい行が足されるだけで古い行はそのマシンに
残り、同じ matcher の下で両方が走る(id そのものを変える更新は、突き合わせから見れば削除ではなく
追加になる)。綴りを変えたときは古い方を手で消してもらう必要がある。

JSON 側はマージを通すと `json.dumps` の整形に正規化される。WT の settings.json は `"actions":`
と `[` を別の行に置く独特の書き方をしており、それが通常の形に均されるため、最初の1回だけ diff が
全面的になる(値は変わらないので実害は無く、2回目以降は安定する)。
...
- **TOML を自前のミニパーサで読み書きする(完全なパーサを目指す)**: 対応構文が増えるほど、
  ...
  読み、書く前に自分のパーサで読み直し、通らなければ dst に触らない。「外れた」に含まれるのは
  `[[array of tables]]` だけでなく、値の綴りが語彙に無いものすべて(§4.7 の2と3の境界)。
  dst を捨てて上書きするのは、どう読んでも TOML が拒む形だと分かったときに限る。コメント・行順・
  改行コードが残るのも、文書を再生成しないこの方式の帰結。
