...
- **配置に失敗した run は空のディレクトリを残すことがある**。配置先のディレクトリも BACKUP_DIR も
  ...
  空のディレクトリが残りうると述べる。
- **stdout を途中で読むのをやめる呼び出し方には耐えない**。`./setup.sh | head -2` は次の書き込みで
  SIGPIPE により run ごと終わる(実測: `PIPESTATUS` は `141 0`。警告も末尾の列挙も出ず、以降の配置も
  走らない)。これは**直さない**: 耐えるには、`set -e` の下で EPIPE がその場で落とすので file 内の
  すべての `echo` に失敗処理を付けるか、報告を溜めて最後に一度に書くしかなく、後者は「失敗した
  コマンド自身の stderr のすぐ下に、それを説明する WARNING が出る」という現在の並びを壊す。run は
  非0で終わり、配置は冪等なので次の run が同じ判定をやり直して収束する — 途中まで適用されたまま
  忘れられるものは無い。出力を切り詰めたいときはファイルに落としてから読む。
