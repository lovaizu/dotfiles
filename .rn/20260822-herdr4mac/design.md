...
(`guid`)・`actions`(`id`)・`schemes`(`name`)、Claude Code の `hooks.<Event>`(`matcher`)。
いずれも「WT が WSL ディストロごとにプロファイルを、UI のキー割り当てごとに action を書き足す」
「Claude Code が `/config` で設定したフックを書き足す」ので、配列を葉として置換すると
そのマシンにしか無い要素が消える。それ以外の配列は葉のまま(消えるときは警告を出す)。書き込みは同一ディレクトリの
...
`[[array of tables]]` は非対応(管理対象の config に該当なし)。id で突き合わせる4つの配列
以外は葉として丸ごと置き換わるので、dst 固有の要素は消える — 消えるときは警告を出す。
`hooks.<Event>` の要素の中の `hooks` 配列は葉のまま(1つの matcher に対する定義そのものは
dotfiles が持ち主なので、要素単位で混ぜる先ではない)。JSONC のコメントは JSON マージを通ると
失われる(同上)。
