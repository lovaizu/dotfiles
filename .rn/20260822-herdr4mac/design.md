...
2. **dst が壊れている** — バックアップを取り、dotfiles の内容で復旧し、失ったものとバックアップの
   位置を `WARNING:` で伝えて続行する。報告は**書き込みが成功したあと**に出す(書けなかったときに
   「置き換えました」と言わないため)。
3. **マージできない**(dst 側に自前パーサの語彙に無いものがある、書き込めない、出力の自己検査に
   落ちた)— dst を一切書かずに警告して続行する。「dst に不正な内容を書かない」は dst を触らない
   ことで完全に達成でき、1台の壊れた設定が他の全ファイルの配置を止める理由は無い。

2と3の境界は**「文書そのものが不正」か「読めなかったのは自前パーサの側」か**で引く。終端しない
文字列・閉じない括弧・キーでもヘッダでもない行・キーの二重定義・テーブルの二重宣言は、どう読んでも
TOML / JSON が拒む形なので結末2。逆に、値の綴りが語彙に無い(TOML 1.1 の構文、`datetime` が持てない
うるう秒、`[[array of tables]]`)ものは結末3 — 自前パーサの不足を「あなたの設定が壊れている」と
言い換えて破棄しないため。判断できないものは安全側(触らない)に倒す。終了コードはこの3つを
0 / 3 / 4 / 5 / 6 の5通りに割ったもの(成功は「マージした」と「新規に置いた」に、結末3は「dst は
そのまま」と「dst が無く配置もできなかった」に分かれる)。

破れの検出: 書き込む前に必ず自分のパーサで読み直し、キーの重複・テーブルの二重宣言・キーとテーブルの
衝突・値の構文エラーがあれば書かない(=結末3)。ここでの「書かない」はマージ**出力**の話で、dst
自体の扱いは上の2と3の境界で決まる。加えてタスク検証で、実物の `~/.config/herdr/config.toml`
