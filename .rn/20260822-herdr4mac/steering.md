...
- README が**再現に要るものだけ**を書いている(#19 で基準を引き直した。旧: 「意図と規則だけ」
  + 操作 / Win キー / Mac キー のキー対応表)。残すのは3種類 — (a) 設定対象(何をどのファイルが
  持つか)、(b) 設定ファイルを素直に読むと欠陥に見えるが意図している点、(c) `setup.sh` が実行
  できず人がやるしかない手順。**具体的な設定値は書かない**(フォント名とサイズ・色の hex・
  テーマ名・送信バイト列はすべて設定ファイル側にしか置かない)。**キー対応表と管理対象/配置先の
  表も置かない** — 再現は `setup.sh` がやるので表は在庫であり、両OSで指運びを揃えている旨の
  1行が (b) として残れば統一が崩れたことには気づける。`setup.sh` が実行時に表示する内容
  (配置先・退避・symlink・失敗時の扱い)も置かない(2か所になるため)
...
### #17: README に戻ってきた重複を落とす

**Purpose**: #8 は README から「値」を落としたが、線を**値**に引いたため**重複**が残った。フォントの
正体は `font-hackgen-nerd`(cask 名)と「`_NF` が付くアーカイブ / Console 版の ttf」という形で戻って
きており、フォントを変えれば README も直すことになる — #8 が直したはずの故障が名前を変えて残って
いる。WT のプロファイル名の列挙は在庫そのもので、1つ増やせば古くなる(しかもその文が言いたい
「`profiles.defaults` に置けば全プロファイルが継承する」に名前は1つも要らない)。ユーザーの指摘。

**Completion criteria**:

- README からフォントの正体(名前・cask 名・アーカイブの命名規則)が復元できない。フォントを
  変えたときに直す場所が設定ファイルだけになる
- README に WT のプロファイル名の列挙が無い
- 手順として必要な記述は失われていない — フォントを手で入れる読者が、何をどこから入手すればよいか
  README と設定ファイルだけで辿れる

レビュー: `checks/17.md`。専門家レビューは回さず、コーディネータが実測で確認した(Rules)。
落としたのは3つ — アーカイブの命名規則、cask 名(`setup.sh` が自分で打ち、失敗時の warn が
再実行コマンドを表示するので読者は打たない)、WT のプロファイル名の列挙。

**初稿は行き過ぎて、参照が輪になった。** 入手先 URL まで落としたため、`setup.sh` の2つの warn が
`install the font by hand (see README)` と送った先の README が入手先を持たず、手で入れる読者が
どこにも着かなくなっていた。コーディネータが差し戻し、リンクを1本だけ戻した(`34613a1`)。
落とす対象は「変更したら同期が必要になる**値**」であって、**配布元へのリンクはポインタ**である、
という線に引き直した。

**受け入れた結合を2件記録した**(どちらも外部の都合で勝手に古くなる): 配布元 URL(フォントを
変えれば1行直す)と iTerm2 のメニュー名(`setup.sh` 側は変数1つに集約済みなので直すのは2か所)。


### #18: setup.sh のコメントを、消すと壊れるものだけにする

**Purpose**: `setup.sh` は 865 行のうち**コード 261 行・コメント 582 行**(2.2 倍)。中身は #10 の
レビュー3巡・確定指摘44件の副産物で、「指摘は真だがコードは変えない」と判断するたびに答えを
コメントとして置いた結果になっている。実測の記録が42件、採らなかった代案の理由が38件。最大の塊5つ
(計207行)は design.md の方針・分類の再掲で、**コメント自身が「§4.6 of design.md carries the same
three」と申告している** — README で問題にしたのと同じ重複が、向きを変えて存在している。
#10 でレビューが同じ指摘を挙げ、コーディネータが「実測の記録として残す価値が上回る」として
却下したが、持ち主が同じことを言うならその費用判断は成り立たない。

**Completion criteria**:

- 残ったコメントがすべて「**その行がなぜそうなのか**」を言っている。「何をしているか」の言い換え、
  design.md にある方針・分類の再掲、レビュー履歴の記録は残っていない
- **コードが1バイトも変わっていない**: 変更前後で `grep -vE '^\s*#' setup.sh` の結果が完全一致
- `bash -n` と `shellcheck -s bash` が通る。隔離ホームで2回流して exit 0 / 0、2回目は `Up to date`、
  配置されたファイルが dotfiles と `diff` 一致し、**出力が変更前と同一**
- design.md / steering.md と矛盾しない

レビュー: `checks/18.md`。専門家レビューは回さず、コーディネータが4つの検証を再実行して確認した(Rules)。
**総 865 → 524 行、コメント 582 → 241 行、コードは 261 行のまま1バイトも変わっていない**
(`grep -vE '^\s*#'` の結果が変更前と完全一致)。`bash -n` / `shellcheck` は通り、隔離ホームで2回流した
出力が変更前と `diff` 一致、配置されたファイルも原本と一致。最大のコメント塊は 66行 → 15行。

落としたのは design.md の再掲(通知3分類・`deploy` の方針・XDG の方針・stdout/stderr の分け方ほか)で、
落とす前に §4.5 / §4.6 に実在することを1件ずつ確認した。**実在しなかった2件は design.md へ移した** —
変数名の大小文字規約と、半端な TOML/JSON の被害の実測(行境界20通りすべて `config: ok`、うち15通りが
`[keys]` 欠落。rename を採る根拠が甘い側の読みに乗っていることを言う唯一の記録だった)。

残したのは「その行を消すか、もっともらしい単純な書き方に直すと実測済みの欠陥が戻る」もの66件。
関数頭の「何をする関数か」1〜2行はこの規則を満たさないが、setup.sh 単体でコードを追える状態を
保つために残した。


### #19: README を「再現に要るもの」だけにする

**Purpose**: #8 は README から「値」を、#17 は残った「重複」を落としたが、どちらも**線の引き方**で
判断していた。目的(clone → setup 一発で端末環境を再現する)から引き直すと、残す理由があるのは
「それが無いと再現できない」か「それが無いと次に触った人が再現を静かに壊す」ものだけになる。
配置先の表・退避の仕様・symlink の扱い・失敗時の挙動は setup.sh が実行時に自分で喋るので、
README に無くても再現は1回で通る。キー対応表は再現に要らない在庫。ユーザーの指摘
「成果物から読み取れることは書かなくてもよい」「設定対象と設定目的ぐらい」。

**Completion criteria**:

- README に残るのは3種類だけ — (a) 設定対象(何をどのファイルが持つか)、(b) 成果物からは
  欠陥に見えるが意図している点、(c) setup.sh が実行できず人がやるしかない手順
- setup.sh が実行時に表示する内容(配置先・退避・symlink・失敗時の扱い)が README に無い
- キー対応表と、管理対象/配置先の表が無い
- setup.sh の warn が README へ送る2件(Win のフォント手動 / Mac の brew 不在・失敗)が
  README で行き止まりにならない — 入手先と、名前が載っている設定ファイルに辿り着ける
- Mac の初回手作業(herdr プロファイルをデフォルトにする)の手順が残っている

レビュー: `checks/19.md`。専門家レビューは回さず、コーディネータが実測で確認した(Rules)。
**95 → 46 行。**落としたのはキー対応表、管理対象/配置先の表、退避・symlink・失敗時・XDG の説明、
`profiles.defaults` の継承の話、シェルを統一しない旨、README 自身の規則の宣言。
残した (b) は3件 — 端末が暗く herdr UI が明るいのは意図であること、Mac のキーが `⌘` 側なのは
HHKB の物理位置に合わせているから、丸ごと上書きで配置先の設定は消えること。いずれも
**設定ファイルを素直に読むと「直すべき欠陥」に見える**ものだけを残した。

**#17 で引いた線(値かポインタか)より強い基準になっている。**#17 が「受け入れた結合」として
記録した iTerm2 のメニュー名は、Mac の初回手作業として (c) に該当するため残る。配布元 URL も
(c) の入手先として残る。

### #20: setup.sh のコメントを「再現を静かに壊すもの」だけにする

**Purpose**: #18 は「消すと実測済みの欠陥が戻る」で 66 件を残したが、この基準は目的に錨を
下ろしていない — 実測が残っていることと、それが再現に効くことは別。目的から引き直すと、残す
理由があるのは「消して素直な書き方に直すと、**再現に失敗したまま exit 0 で終わる**」ものだけ。
実行時に `record_failure` が同じことを喋って exit 1 で騒ぐ箇所(WSL interop ほか)は、コメントが
無くても失敗が見える。ユーザーの指摘「実装みて自明なことは書いてないよね?」。

**Completion criteria**:

- 残ったコメントがすべて「消して素直な書き方に直すと、再現に失敗したまま成功として終わる」
  箇所に付いている。実行時のメッセージが同じことを言っている説明、関数の要約、自明な言い換え、
  出力ストリームや文言の選択の理由は残っていない
- コードが変わっていない: 非コメント・非空行の並びが変更前と完全一致
- `bash -n` と `shellcheck -s bash` が通る。隔離ホームで2回流して exit 0 / 0、2回目は `Up to date`、
  配置されたファイルが dotfiles と `diff` 一致し、**出力が変更前と同一**
- 配置失敗を作った run が 警告 → 続行 → 末尾で列挙 → exit 1 のまま

レビュー: `checks/20.md`。専門家レビューは回さず、コーディネータが実測で確認した(Rules)。
**総 524 → 367 行、コメント 240 → 83 行、コードは 261 行のまま不変。**残ったのは 21 塊(66 塊から 45 塊を削除)。
落とした最大の塊は WSL interop の説明(23行)で、これは `record_failure` に渡す文字列が
同じ内容をユーザーに喋り、exit 1 で騒ぐ — コメントと実行時メッセージの2か所だった。
#18 が「読む人が最初に要る情報」として意図的に残した関数頭の要約も、**追えることは再現に
効かない**ので落とした。

**#18 の判断を1件だけ覆している** — 「実測の記録として残す価値が上回る」で残した litter 系
(`sweep_tmp_files` の位置、一時ファイルの掃除、subshell と trap)は、消えて困るのがゴミの
残留であって再現ではないので落とした。逆に #18 が同列に扱っていた silent-success 系
(`REACHED_END`、`deploy` が常に 0、退避パスを stdout で返す、`mv -f`、`%LOCALAPPDATA%` の
