...
- [ ] フォントの正体を復元できる記述を落とす。入手先 URL は手順として必要なので、URL をどう扱うか
      (残す/変える)を決めて理由を記録する
- [ ] WT のプロファイル名の列挙を落とす
- [ ] 残した重複(配置先パス・設定項目名・キー対応表)について、なぜ割に合うのかを checks に記録する
- [ ] self-check (OK/NG per completion criterion, record in checks/17.md)
