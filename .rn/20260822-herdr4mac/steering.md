...
- [ ] コメントを2分する: **消すと実測済みの欠陥が戻るもの**(`mv -f` の tty プロンプト、一時ファイルの
      先頭ドット、trap と一時ファイル名の受け渡し、`[ -e "$dst" ]` など)と、**design.md にある方針・
      分類の再掲**
- [ ] 後者を落とす。落とす前に、その内容が design.md に実在することを1件ずつ確かめる — 無ければ
      design.md へ先に移す(setup.sh を読むだけでコードが追えることは保ち、design.md への参照が
      無いと理解できない状態にはしない)
- [ ] 挙動が変わっていないことを機械的に示す(下記の完了基準)
- [ ] self-check (OK/NG per completion criterion, record in checks/18.md)
- [ ] コーディネータの確認(専門家レビューは回さない — Rules のとおり)
